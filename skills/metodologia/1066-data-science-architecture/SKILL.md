...
description: 
  ML/AI system design — model lifecycle, feature stores, experiment tracking, model serving, MLOps pipelines. [EXPLICIT]
  Use when the user asks to "design an ML system", "architect model serving", "set up experiment tracking",
  "design feature store", "plan MLOps pipeline", or mentions model registry, A/B testing, drift detection, or retraining triggers. [EXPLICIT]
...
1. **Mandatory reproducibility.** Every experiment, every training run, every feature pipeline must be reproducible. If it cannot be reproduced, it cannot be trusted. [EXPLICIT]
2. **Monitoring > accuracy.** A model with 95% accuracy that drifts without detection is worse than one with 85% that is monitored. Drift detection is day 1. [EXPLICIT]
3. **Feature store = single source of truth.** Features shared between models, versioned, with lineage. Do not reinvent features per model. [EXPLICIT]
...
- `{MODO}`: `piloto-auto` (default) | `desatendido` | `supervisado` | `paso-a-paso`
  ...
  - **desatendido**: Zero interruptions. Arquitectura ML documentada automáticamente. Assumptions documented. [EXPLICIT]
...
```
HTML self-contained branded (Design System MetodologIA v5). Light-First Technical. Incluye MLOps maturity level visual, feature store comparison matrix interactiva, y responsible AI checklist con estado por item. WCAG AA, responsive, print-ready. [EXPLICIT]
