#!/usr/bin/env python3
"""
Typo Detector: Surface-level error detection and correction for Pass 1.

Detects and corrects common spelling errors, typos, autocorrect artifacts,
dyslexia patterns, and punctuation issues using pattern matching and
dictionary-based approaches. Does not use external spell-check libraries
to remain dependency-free and portable.

Detection strategy (in priority order):
1. HIGH confidence: exact dictionary match (teh→the, recieve→receive)
2. HIGH confidence: missing apostrophe contractions (dont→don't)
3. MEDIUM confidence: QWERTY neighbor substitution (wirking→working)
4. MEDIUM confidence: missing vowel reconstruction (imprtnt→important)
5. LOW confidence: phonetic/homophone disambiguation (there/their/they're)

Design decision: conservative by default. Only auto-corrects HIGH confidence
matches. MEDIUM corrections are flagged as suggestions. LOW corrections are
reported but not applied. A wrong correction is worse than leaving the typo.

Usage:
    python typo-detector.py "raw text here"               # Human-readable output
    python typo-detector.py "raw text here" --json         # JSON for integration
    python typo-detector.py "raw text here" --language es  # Language hint

Exit codes:
    0: Success (corrections may or may not have been made)
