...
Runs all five analysis passes on raw user input:
- Pass 1: Surface analysis (typos, spelling, punctuation)
- Pass 2: Five Whys (root cause analysis)
- Pass 3: Seven So Whats (value chain analysis)
- Pass 4: Intent analysis (gap detection)
- Pass 5: Reformulation (synthesize into quality prompt)

Usage:
    python text-analyzer.py "raw user input text"
    python text-analyzer.py "input" --passes 1,4,5
    python text-analyzer.py "input" --json
    python text-analyzer.py "input" --language es
    python text-analyzer.py "input" --passes 1,2,3 --json
"""
